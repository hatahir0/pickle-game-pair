...
export function generateSchedule(
  ...
  const maxCourts = Math.max(1, Math.min(courts, Math.floor(numPlayers / 4)))

  const partner = matrix(numPlayers)
  const opponent = matrix(numPlayers)
  const restCount = Array(numPlayers).fill(0)
  const playCount = Array(numPlayers).fill(0)
  let prevFoursomes = new Set<string>()
  let prevMates = matrix(numPlayers)

  const rounds: Round[] = []

  for (let roundIdx = 0; roundIdx < totalGames; roundIdx++) {
    const gamesThisRound = maxCourts
    const needed = gamesThisRound * 4

    const order = shuffle([...Array(numPlayers).keys()], rng)
    order.sort((a, b) => restCount[a] - restCount[b] || playCount[b] - playCount[a])
    const resting = order.slice(0, numPlayers - needed).sort((a, b) => a - b)
    const playing = order.slice(numPlayers - needed)
    ...
    const newMates = matrix(numPlayers)
    ...
    prevFoursomes = newFoursomes
    prevMates = newMates
    rounds.push({ games: committed, resting })
  }
